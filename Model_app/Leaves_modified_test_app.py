...
recommendations = {
    ...
    # This key matches the class name from the confusion matrix
    'Potato_healthy_aug': { 
     ...
     # Added this key from your original file as a fallback
    'Potato___healthy': { 
        "disease_name": "Healthy Potato",
        "why": "Leaves are clean, green, and free from disease lesions.",
        "causes": "N/A - Plant is healthy.",
...
if uploaded_file is None:
    ...
    with col1:
        st.image("https://i.imgur.com/8YlB0mN.jpg", caption="Corn (Common Rust)", use_column_width=True)
    with col2:
        st.image("https://i.imgur.com/fFeoJ1t.jpg", caption="Potato (Early Blight)", use_column_width=True)
    with col3:
        st.image("https://i.imgur.com/K6KqY0q.jpg", caption="Tomato (Late Blight)", use_column_width=True)
    ...
    with st.spinner('Analyzing your leaf... 🍃'):
        ...
        # Get the correct recommendation.
    ...
    # Animation for the "vibe"!
    st.balloons()

    # --- 7. Display Results (Columns & Tabs) ---
    col1, col2 = st.columns([1, 2]) # Image in col1, Info in col2

    with col1:
        st.image(image, caption="Your Uploaded Leaf", use_column_width=True)
    ...
    with col2:
        ...
        with tab2:
            ...
            st.warning(content['controls']) # .warning gives it a yellow box
